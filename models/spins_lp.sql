...
select product_level ,subcategory ,brand , product_universe , upc , time_period ,time_period_end_date, geography , positioning_group , category , company
			...
			, sum(cast(coalesce(dollars,0) as float)) as sales
			, sum(cast(coalesce(dollars_2_yago,0) as float)) as sales_2_yago
			, sum(cast(coalesce(dollars_yago,0) as float)) as sales_yago
...
select * 
	...
	, cast(sales as float)							/nullif(cast(sales_2_yago				as float),0) -1 sales_per_change_2_yago
	, cast(sales as float)							/nullif(cast(sales_yago					as float),0) -1 sales_per_change_yago
